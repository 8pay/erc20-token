...
const deployWithoutLGE = deployments.createFixture(async ({ deployments, ethers }, options) => {
  ...
  const pad = await ethers.getContract("EightPayToken");
...
describe("EightPayToken", () => {
  describe("Deployment", () => {
    it("name should be 8PAY Network", async () => {
      const { pad } = await deployWithLGE();
      expect(await pad.name()).to.be.equal("8PAY Network");
    });

    it("symbol should be 8PAY", async () => {
      const { pad } = await deployWithLGE();
      expect(await pad.symbol()).to.be.equal("8PAY");
    });
    ...
    it("total supply should be 88 888 888 tokens", async () => {
      const { pad } = await deployWithLGE();
      expect(await pad.totalSupply()).to.be.equal(tokens("88888888"));
    });

    it("deployer should have the total initial supply", async () => {
      const { pad, deployer } = await deployWithLGE();
      expect(await pad.balanceOf(deployer.address)).to.be.equal(tokens("88888888"));
    });
  });
  ...
  describe("allowance", () => {
    it("allowance works as expected", async () => {
      ...
      await expect(pad.decreaseAllowance(holder5.address, tokens("5"))).to.be.revertedWith("ERC20: decreased allowance below zero");
      expect(await pad.allowance(deployer.address, holder5.address)).to.equal(tokens("4"));
    });
  });
